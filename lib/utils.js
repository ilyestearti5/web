...
export function readDirection(element, ignoreTags) {
    ...
    for (let i = 0; i < attrs.length; i++) {
        ...
        Object.defineProperty(attributes, a, {
            value: attrs.getNamedItem(a)?.nodeValue,
            enumerable: true,
            configurable: true,
            writable: false,
        });
    ...
    return {
        ...
        inner: childs
            .filter(({ tagName }) => !ignoreTags.includes(tagName.toLowerCase()))
            .map(ele => readDirection(ele, ignoreTags)),
        async fn(e) { },
    };
}
export const _ = (query, result) => (result == 'all'
    ? Array.from(document.querySelectorAll(query))
    : document.querySelector(query));
export function visiblity(element) {
    return {
        get visible() {
