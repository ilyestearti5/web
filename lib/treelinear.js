...
var _TreeLinear_instances, _a, _TreeLinear_mainTreeElement, _TreeLinear_callbackQuery, _TreeLinear_trees, _TreeLinear_closeIconElement, _TreeLinear_openIconElement, _TreeLinear_isLine, _TreeLinear_inner, _TreeLinear_outer, _TreeLinear_toQuery, _TreeLinear_toElement, _TreeLinear_getLevelElement, _TreeLinear_getIconElement, _TreeLinear_isOpend, _TreeLinear_isClosed, _TreeLinear_open, _TreeLinear_close, _TreeLinear_toggle;
...
export class TreeLinear extends Itr {
    constructor(root, title, propertys, defaultValues) {
        ...
        this.treePropertys = [];
        this.levelSize = 20;
        _TreeLinear_closeIconElement.set(this, [
            'i',
            '&rightarrow;',
            {
        ...
        _TreeLinear_openIconElement.set(this, [
            'i',
            '&downarrow;',
            {
                style: 'cursor: pointer',
        ...
        __classPrivateFieldGet(TreeLinear, _a, "f", _TreeLinear_trees).push(this);
    }
    get ITEMS() {
        return super.ITEMS;
    }
    ...
    changeLevel(element, to) {
        var levelElement = __classPrivateFieldGet(this, _TreeLinear_instances, "m", _TreeLinear_getLevelElement).call(this, element);
        if (!levelElement)
            return;
    ...
    createRow(input, lvl = 0, closed = false, visible = true) {
        ...
        if (this.isTree(result)) {
            ...
            showMoreIcon.appendChild(crt(...(closed ? __classPrivateFieldGet(this, _TreeLinear_closeIconElement, "f") : __classPrivateFieldGet(this, _TreeLinear_openIconElement, "f"))));
    ...
    async delete(element, timeout, limit) {
        await each(__classPrivateFieldGet(this, _TreeLinear_instances, "m", _TreeLinear_inner).call(this, element), async (ele) => await this.delete(ele, timeout, limit), timeout, limit);
        element.remove();
    }
    async insert(element, tree, timeout, limit) {
        ...
        var isOpend = __classPrivateFieldGet(this, _TreeLinear_instances, "m", _TreeLinear_isOpend).call(this, element);
        await each(tree, async ({ body, innerTree }) => {
    ...
    async sort(element, sortBy, orderby, deep = true, timeout, limit) {
        ...
        var sort = async (tree = Tr) => {
            ...
            if (orderby == 'DESC')
                sortedData = sortedData.reverse();
            await each(sortedData, async (tree) => {
                var ch = childs(tree).map(({ row }) => row);
                await each(ch.reverse(), ele => {
                    element.after(ele);
                }, timeout, limit);
            }, timeout, limit);
            if (deep) {
                await each(tree.filter(({ body: { row } }) => this.isTree(row)), ({ innerTree }) => {
    ...
    async paste(timeout = 5, limit = 1) {
        ...
        if (div >= 1 && div == parseInt(`${div}`))
            await each(selected, async (element, index) => result.push(...(await this.insert(element, data.slice(index * div, (index + 1) * div), timeout, limit))), timeout, limit);
        else if (selected.length)
            await each(selected, async (element) => result.push(...(await this.insert(element, data, timeout, limit))), timeout, limit);
        else
            result.push(...(await this.insert(__classPrivateFieldGet(this, _TreeLinear_mainTreeElement, "f"), data, timeout, limit)));
        this.isloading = false;
        return result;
    }
    setOpenIconElement(config) {
        __classPrivateFieldSet(this, _TreeLinear_openIconElement, config, "f");
        this.ITEMS.forEach(ele => {
            var icon = __classPrivateFieldGet(this, _TreeLinear_instances, "m", _TreeLinear_getIconElement).call(this, ele);
            if (icon) {
                icon.innerHTML = '';
    ...
    getOpenIconElement() {
        return crt(...__classPrivateFieldGet(this, _TreeLinear_openIconElement, "f"));
    }
    setCloseIconElement(config) {
        __classPrivateFieldSet(this, _TreeLinear_closeIconElement, config, "f");
        this.ITEMS.forEach(ele => {
            var icon = __classPrivateFieldGet(this, _TreeLinear_instances, "m", _TreeLinear_getIconElement).call(this, ele);
            if (icon) {
                icon.innerHTML = '';
    ...
    getCloseIconElement() {
        return crt(...__classPrivateFieldGet(this, _TreeLinear_closeIconElement, "f"));
    }
    ...
    static title(title) {
        return __classPrivateFieldGet(this, _a, "f", _TreeLinear_trees).find(({ title: tlt }) => tlt == title) || null;
    }
}
_a = TreeLinear, _TreeLinear_mainTreeElement = new WeakMap(), _TreeLinear_callbackQuery = new WeakMap(), _TreeLinear_closeIconElement = new WeakMap(), _TreeLinear_openIconElement = new WeakMap(), _TreeLinear_instances = new WeakSet(), _TreeLinear_isLine = function _TreeLinear_isLine(element) {
    ...
    var result = [];
    while (nextElementSibling && (__classPrivateFieldGet(this, _TreeLinear_instances, "m", _TreeLinear_isLine).call(this, nextElementSibling) || initLvl < this.getLevel(nextElementSibling))) {
