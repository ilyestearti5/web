...
var _Tree_instances, _Tree_treepropertys, _Tree_callbackQuery, _Tree_mainTreeElement, _Tree_getItemElement, _Tree_getContentElement, _Tree_getInnerTreeElement, _Tree_getOuterTreeElement, _Tree_inner, _Tree_outer, _Tree_toElement, _Tree_toQuery;
...
export class Tree extends Iterations {
    ...
    async prepend(element, data, timeout, limit) {
        ...
        this.isloading = true;
        var subtreeElement = __classPrivateFieldGet(this, _Tree_instances, "m", _Tree_getInnerTreeElement).call(this, element);
        forEachAsync(data.reverse(), input => {
            const ele = this.createRow(input);
    ...
    async insert(element, data, timeout, limit) {
        ...
        this.isloading = true;
        var subtreeElement = __classPrivateFieldGet(this, _Tree_instances, "m", _Tree_getInnerTreeElement).call(this, element);
        forEachAsync(data, async ({ body, innerTree }) => {
            var row = this.createRow(body);
            subtreeElement.appendChild(row);
    ...
    async sort(element, by, orderby, deep = true, timeout, limit) {
        ...
        var innerMainElement = __classPrivateFieldGet(this, _Tree_instances, "m", _Tree_getInnerTreeElement).call(this, element);
        if (innerMainElement) {
            await forEachAsync(readyinners, ele => {
                innerMainElement.appendChild(ele);
            }, timeout, limit);
        }
        if (deep)
            await forEachAsync(innersElement.filter(ele => this.isTree(ele)), async (ele) => await this.sort(ele, by, orderby, true, timeout, limit), timeout, limit);
    }
    ...
    async paste(timeout, limit) {
        ...
        if (div >= 1 && div == parseInt(`${div}`))
            await forEachAsync(selectedElement, () => null, timeout, limit);
...
_Tree_treepropertys = new WeakMap(), _Tree_callbackQuery = new WeakMap(), _Tree_mainTreeElement = new WeakMap(), _Tree_instances = new WeakSet(), _Tree_getItemElement = function _Tree_getItemElement(element) {
